module midpoint_integrator
    ...
    use integrator_midpoint, only: symplectic_integrator_midpoint_t

    implicit none

    type, extends(integrator_t) :: midpoint_integrator_t
        type(symplectic_integrator_data_t) :: si
        type(field_can_data_t) :: f
        class(symplectic_integrator_midpoint_t), allocatable :: sym_integ
        contains
        procedure :: init
        procedure :: timestep
        procedure :: get_field_evaluations
    end type midpoint_integrator_t

    contains

    subroutine init(self, field)
        class(midpoint_integrator_t), intent(inout) :: self
        class(field_can_t), intent(in) :: field

        self%sym_integ = symplectic_integrator_midpoint_t(field)
    end subroutine init
    ...
    function get_field_evaluations(self) result(n)
        class(midpoint_integrator_t), intent(in) :: self
        integer(8) :: n

        n = n_field_evaluations
    end function get_field_evaluations

end module midpoint_integrator
